...
Этот раздел относится к сценарию, где вы используете Codex/OpenAI-аккаунты через CLIProxyAPI.
Запустите podman-build.sh или docker-build.sh, выберите 2 пункт и собирайте образ.
...
Зачем это нужно:
- Поле `model` задаёт модель по умолчанию для обычной интерактивной работы на GPT-backed конфигурации.
...
Если вы используете другие провайдеры через CLIProxyAPI и вам не нужно это сопоставление моделей для Codex/OpenAI, базовая конфигурация Claude Code выглядит так:
